...
*/
	...
	$projcode = ($start+$i)%14;
		$temp = array(
			...
			'ProStartTime'=>"2016-01-01",
			'Stage'=>"备注".(string)($start+$i),
			'SN'=>"SN".(string)($start+$i),
            'PN'=>"PN".(string)($start+$i),
            'Order'=>"订单".(string)($start+$i),
            'Model'=>"Model:".(string)($start+$i),
		);
		array_push($pointtable,$temp);
	}
	$body = array(
...
*/
    ...
    */
    ...
    case "PointLogin":
    	...
    	$jsonencode = _encode($retval);
    	echo $jsonencode; break;
    case "PointMaintenanceList":

        $projdev = array();
        for($i=0;$i<4;$i++){
            $temp = array(
                'repairID'=>"RID".(string)($i+1),
                'repairSN'=>"RSN".(string)($i+1),
                'name'=> "修理工".(string)($i+1),
                'date'=> "11/12/13",
                'action'=> "废物处理",
                'result'=> "砸锅卖铁"
